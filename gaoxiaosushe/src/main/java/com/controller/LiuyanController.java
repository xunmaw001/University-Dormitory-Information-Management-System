...
//            liuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        if("".equals(liuyan.getLiuyanContent()) || "null".equals(liuyan.getLiuyanContent())){
                liuyan.setLiuyanContent(null);
        }
        if("".equals(liuyan.getLiuyanHuifuContent()) || "null".equals(liuyan.getLiuyanHuifuContent())){
                liuyan.setLiuyanHuifuContent(null);
        }
...
//                            liuyanEntity.setLiuyanUuidNumber(data.get(0));                    //留言编号 要改的
//                            liuyanEntity.setLiuyanName(data.get(0));                    //留言标题 要改的
//                            liuyanEntity.setLiuyanFile(data.get(0));                    //附件 要改的
...
//                            liuyanEntity.setLiuyanHuifuContent("");//详情和图片
...
//                            liuyanEntity.setCreateTime(date);//时间
                            ...
                            //把要查询是否重复的字段放入map中
                                //留言编号
                                if(seachFields.containsKey("liuyanUuidNumber")){
                                    List<String> liuyanUuidNumber = seachFields.get("liuyanUuidNumber");
                                    liuyanUuidNumber.add(data.get(0));//要改的
                                }else{
                                    List<String> liuyanUuidNumber = new ArrayList<>();
                                    liuyanUuidNumber.add(data.get(0));//要改的
                                    seachFields.put("liuyanUuidNumber",liuyanUuidNumber);
                                }
                        }

                        //查询是否重复
                         //留言编号
                        List<LiuyanEntity> liuyanEntities_liuyanUuidNumber = liuyanService.selectList(new EntityWrapper<LiuyanEntity>().in("liuyan_uuid_number", seachFields.get("liuyanUuidNumber")));
                        if(liuyanEntities_liuyanUuidNumber.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(LiuyanEntity s:liuyanEntities_liuyanUuidNumber){
                                repeatFields.add(s.getLiuyanUuidNumber());
                            }
                            return R.error(511,"数据库的该表中的 [留言编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R add(@RequestBody LiuyanEntity liuyan, HttpServletRequest request){
        ...
        Wrapper<LiuyanEntity> queryWrapper = new EntityWrapper<LiuyanEntity>()
            .eq("yonghu_id", liuyan.getYonghuId())
            .eq("liuyan_uuid_number", liuyan.getLiuyanUuidNumber())
            .eq("liuyan_name", liuyan.getLiuyanName())
            .eq("liuyan_types", liuyan.getLiuyanTypes())
