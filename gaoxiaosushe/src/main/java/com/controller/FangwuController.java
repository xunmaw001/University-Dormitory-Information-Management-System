    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(fangwu !=null){
            ...
            //级联表 宿舍管理员
            //级联表
            SusheguanliyuanEntity susheguanliyuan = susheguanliyuanService.selectById(fangwu.getSusheguanliyuanId());
    ...
    */
    ...
    public R save(@RequestBody FangwuEntity fangwu, HttpServletRequest request){
        ...
        if(false)
            ...
            fangwu.setSusheguanliyuanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<FangwuEntity> queryWrapper = new EntityWrapper<FangwuEntity>()
            .eq("susheguanliyuan_id", fangwu.getSusheguanliyuanId())
            .eq("fangwu_name", fangwu.getFangwuName())
            .eq("fangwu_address", fangwu.getFangwuAddress())
            .eq("fangwu_types", fangwu.getFangwuTypes())
            .eq("sex_types", fangwu.getSexTypes())
            .eq("fangwu_renshu", fangwu.getFangwuRenshu())
            .eq("fangwu_yizhurenshu", fangwu.getFangwuYizhurenshu())
            .eq("shangxia_types", fangwu.getShangxiaTypes())
            .eq("fangwu_delete", 1)
            ;
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            FangwuEntity fangwuEntity = new FangwuEntity();
//                            fangwuEntity.setSusheguanliyuanId(Integer.valueOf(data.get(0)));   //宿舍管理员 要改的
//                            fangwuEntity.setFangwuName(data.get(0));                    //房屋名称 要改的
//                            fangwuEntity.setFangwuUuidNumber(data.get(0));                    //公寓编号 要改的
//                            fangwuEntity.setFangwuPhoto("");//详情和图片
//                            fangwuEntity.setFangwuAddress(data.get(0));                    //房屋位置 要改的
//                            fangwuEntity.setFangwuTypes(Integer.valueOf(data.get(0)));   //公寓类型 要改的
//                            fangwuEntity.setFangwuNewMoney(data.get(0));                    //房租/月 要改的
//                            fangwuEntity.setSexTypes(Integer.valueOf(data.get(0)));   //性别 要改的
//                            fangwuEntity.setFangwuRenshu(Integer.valueOf(data.get(0)));   //限制人数 要改的
//                            fangwuEntity.setFangwuYizhurenshu(Integer.valueOf(data.get(0)));   //已住人数 要改的
//                            fangwuEntity.setFangwuContent("");//详情和图片
//                            fangwuEntity.setShangxiaTypes(Integer.valueOf(data.get(0)));   //是否上架 要改的
...
//                            fangwuEntity.setCreateTime(date);//时间
                        ...
                        if(fangwuEntities_fangwuUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [公寓编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R gexingtuijian(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils pageUtils = fangwuCollectionService.queryPage(params1);
        List<FangwuCollectionView> collectionViewsList =(List<FangwuCollectionView>)pageUtils.getList();
        Map<Integer,Integer> typeMap=new HashMap<>();//购买的类型list
        for(FangwuCollectionView collectionView:collectionViewsList){
            Integer fangwuTypes = collectionView.getFangwuTypes();
