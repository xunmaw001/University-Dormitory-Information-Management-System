	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('fangwuYuyue', this.fangwuYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fangwuYuyuePhoto ? this.detail.fangwuYuyuePhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('fangwuYuyueId', _this.fangwuYuyueId);
                                // _this.$utils.jump('/pages/fangwuYuyueYuyue/add-or-update');

								await _this.$api.add('fangwuYuyueYuyue', {
									yonghuId: _this.user.id,
									fangwuYuyueId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/fangwuYuyueYuyue/list');
							}
						}
					});
				},
