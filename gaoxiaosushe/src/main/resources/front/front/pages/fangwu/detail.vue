		...
		<view class="">
			<!-- <view class="price ">
				<view class="pricejiner"
					  :style="detail.fangwuNewMoney>1000? 'width: 450rpx;':detail.fangwuNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.fangwuNewMoney}}</text>
				</view>
			</view> -->
            ...
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<view  v-if="detail.fangwuRenshu>detail.fangwuYizhurenshu" @tap="baoming" style="display: flex; width: 80%;color: #FFFFFF;">
	...
	<script>
		export default {
			...
			methods: {
				...
				// 收藏
				async getfangwuCollection() {
					let params = {
						page: 1,
						limit: 1,
						fangwuId: this.fangwuId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`fangwuCollection`, params);
					if (res.data.list.length == 1) {
						let fangwuCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('fangwuCollection', JSON.stringify([fangwuCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getfangwuCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fangwuCollection', {
									yonghuId: _this.user.id,
									fangwuId: _this.detail.id,
									fangwuCollectionTypes:1
								});
