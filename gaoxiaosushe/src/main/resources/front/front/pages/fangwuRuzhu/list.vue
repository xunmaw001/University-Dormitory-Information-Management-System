<template>
	...
	<!-- <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('fangwuRuzhu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('fangwuRuzhu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.fangwuRuzhuName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('fangwuRuzhu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">最大人数</text>
                    <text style="width: 70%">{{item.fangwuRenshu}}</text>
                </view>
                <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">已入住人数</text>
                    <text style="width: 70%">{{item.fangwuYizhurenshu}}</text>
                </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('fangwuRuzhu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
				
				<!--<view @tap="tuisu(item.fangwuId)" style="display: flex; width: 80%;color: #FFFFFF;">
                ...
                <view class="" style="display: flex; width: 70%;color: #FFFFFF;">
                    <view @tap="tuisu(item.fangwuId)" :style="[{backgroundImage:'linear-gradient(40deg,'+ btnColor[0] +','+ btnColor[1] +')'}]" style="width:47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-left-radius: 40rpx;border-top-left-radius: 40rpx;">
                        申请退宿
                    </view>
                    <view @tap="tiaosu()" :style="[{backgroundImage:'linear-gradient(130deg,'+ btnColor[1] +','+ btnColor[0] +')'}]" style="width: 47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-right-radius: 40rpx;border-top-right-radius: 40rpx;">
                        申请调宿
                    </view>
                </view>
                <!--<text>请耐心等待审核</text>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`fangwuRuzhu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fangwuRuzhu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.fangwuRuzhuShenheTime=shijian;
                            await _this.$api.update(`fangwuRuzhu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
            ...
            tuisu(fangwuId) {
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
						...
						_this.$utils.jump('/pages/fangwuTuisu/add-or-update');
                    }
                });
            },
            tiaosu() {
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
						// uni.setStorageSync('fangwuId', fangwuId);
						_this.$utils.jump('/pages/tiaosushenqing/add-or-update');
                    }
                });
            },
			// 搜索
			async search() {
                ...
                let res = await this.$api.page(`fangwuRuzhu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
