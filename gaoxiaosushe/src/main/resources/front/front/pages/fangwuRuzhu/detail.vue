	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('fangwuRuzhu', this.fangwuRuzhuId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fangwuRuzhuPhoto ? this.detail.fangwuRuzhuPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('fangwuRuzhuId', _this.fangwuRuzhuId);
                                // _this.$utils.jump('/pages/fangwuRuzhuYuyue/add-or-update');

								await _this.$api.add('fangwuRuzhuYuyue', {
									yonghuId: _this.user.id,
									fangwuRuzhuId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/fangwuRuzhuYuyue/list');
							}
						}
					});
				},
