	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.fangwuCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('fangwuCollection', this.fangwuCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fangwuCollectionPhoto ? this.detail.fangwuCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('fangwuCollectionId', _this.fangwuCollectionId);
                                // _this.$utils.jump('/pages/fangwuCollectionYuyue/add-or-update');

								await _this.$api.add('fangwuCollectionYuyue', {
									yonghuId: _this.user.id,
									fangwuCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/fangwuCollectionYuyue/list');
							}
						}
					});
				},
