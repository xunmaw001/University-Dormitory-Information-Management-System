<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.tiaosushenqingName" type="text" placeholder="申请标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('tiaosushenqing','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('tiaosushenqing','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='tiaosushenqingTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in tiaosushenqingTypesList" :key="index" :class="tiaosushenqingTypes===item.codeIndex?'tabActive':''"
                                  @tap="tiaosushenqingTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.tiaosushenqingName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('tiaosushenqing','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <view v-show="item.fangwuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">房屋名称</text>
                    <text style="width: 70%">{{item.fangwuName}}</text>
                </view>
                <view v-show="item.fangwuTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">公寓类型</text>
                    <text style="width: 70%">{{item.fangwuValue}}</text>
                </view>
                <!-- 当前表 -->
                 <view v-show="item.tiaosushenqingUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">调宿申请编号</text>
                     <text style="width: 70%">
                         <text v-if="item.tiaosushenqingUuidNumber != null">{{item.tiaosushenqingUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.tiaosushenqingFile" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">申请附件</text>
                     <view style="width: 70%" @tap="download(baseUrl+item.tiaosushenqingFile)" class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>下载</view>
                 </view>
                 <view v-show="item.tiaosushenqingValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">调宿申请类型</text>
                     <text style="width: 70%">{{item.tiaosushenqingValue}}</text>
                 </view>
                 <view v-show="item.tiaosushenqingYuanAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">原宿舍位置</text>
                     <text style="width: 70%">
                         <text v-if="item.tiaosushenqingYuanAddress != null">{{item.tiaosushenqingYuanAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.tiaosushenqingContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">申请缘由</text>
                    <view style="width: 70%" v-html="item.tiaosushenqingContent"></view>
                </view>
                 <view v-show="item.tiaosushenqingYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">申请状态</text>
                     <text style="width: 70%">{{item.tiaosushenqingYesnoValue}}</text>
                 </view>
                <view v-show="item.tiaosushenqingYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核意见</text>
                    <view style="width: 70%" v-html="item.tiaosushenqingYesnoText"></view>
                </view>
                <view v-show="item.tiaosushenqingShenheTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核时间</text>
                    <text style="width: 70%">
                        <text v-if="item.tiaosushenqingShenheTime != null">{{item.tiaosushenqingShenheTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('tiaosushenqing','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('tiaosushenqing','审核') && item.tiaosushenqingYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.tiaosushenqingTypesSelect()
		...
		methods: {
            ...
            //当前表的 调宿申请类型 字段 字典表查询方法
            async tiaosushenqingTypesSelect(){
                let tiaosushenqingTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'tiaosushenqing_types',
                });
                tiaosushenqingTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.tiaosushenqingTypesList = tiaosushenqingTypes.data.list
            },
            //当前表的 调宿申请类型字段类别搜索
			tiaosushenqingTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.tiaosushenqingTypes != 0) {
                    params.tiaosushenqingTypes = this.tiaosushenqingTypes
                }
                let _this = this
                let res = await this.$api.page(`tiaosushenqing`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tiaosushenqing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.tiaosushenqingShenheTime=shijian;
                            await _this.$api.update(`tiaosushenqing`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.tiaosushenqingName) {
                    searchForm['tiaosushenqingName'] = this.searchForm.tiaosushenqingName
                }

                let res = await this.$api.page(`tiaosushenqing`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.tiaosushenqingContent!=null){
                        item.tiaosushenqingContent = item.tiaosushenqingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
