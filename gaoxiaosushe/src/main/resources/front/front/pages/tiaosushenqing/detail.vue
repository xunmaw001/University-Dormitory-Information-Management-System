	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.tiaosushenqingId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('tiaosushenqing', this.tiaosushenqingId);
                    this.detail = res.data
					this.detail.tiaosushenqingContent = this.detail.tiaosushenqingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.tiaosushenqingPhoto ? this.detail.tiaosushenqingPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('tiaosushenqingId', _this.tiaosushenqingId);
                                // _this.$utils.jump('/pages/tiaosushenqingYuyue/add-or-update');

								await _this.$api.add('tiaosushenqingYuyue', {
									yonghuId: _this.user.id,
									tiaosushenqingId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/tiaosushenqingYuyue/list');
							}
						}
					});
				},
