<template>
	...
	<!-- <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('weisheng','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('weisheng','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='weishengTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in weishengTypesList" :key="index" :class="weishengTypes===item.codeIndex?'tabActive':''"
                                  @tap="weishengTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.weishengName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('weisheng','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.susheguanliyuanName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">宿舍管理员姓名</text>
                    <text style="width: 70%">{{item.susheguanliyuanName}}</text>
                </view>
                ...
                <!-- 当前表 -->
                 <view v-show="item.weishengUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">卫生编号</text>
                     <text style="width: 70%">
                         <text v-if="item.weishengUuidNumber != null">{{item.weishengUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.weishengValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">卫生状况</text>
                     <text style="width: 70%">{{item.weishengValue}}</text>
                 </view>
                 <view v-show="item.weishengPrice" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">得分</text>
                     <text style="width: 70%">{{item.weishengPrice}}</text>
                 </view>
                <view v-show="item.weishengContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">卫生备注</text>
                    <view style="width: 70%" v-html="item.weishengContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('weisheng','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('weisheng','审核') && item.weishengYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 卫生状况 字段 字典表查询方法
            async weishengTypesSelect(){
                let weishengTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'weisheng_types',
                });
                weishengTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.weishengTypesList = weishengTypes.data.list
            },
            //当前表的 卫生状况字段类别搜索
			weishengTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.weishengTypes != 0) {
                    params.weishengTypes = this.weishengTypes
                }
                let _this = this
                let res = await this.$api.list(`weisheng`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('weisheng', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.weishengShenheTime=shijian;
                            await _this.$api.update(`weisheng`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.list(`weisheng`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.weishengContent!=null){
                        item.weishengContent = item.weishengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
