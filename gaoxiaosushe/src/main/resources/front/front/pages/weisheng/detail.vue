	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('weisheng', this.weishengId);
                    this.detail = res.data
					this.detail.weishengContent = this.detail.weishengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.weishengPhoto ? this.detail.weishengPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('weishengId', _this.weishengId);
                                // _this.$utils.jump('/pages/weishengYuyue/add-or-update');

								await _this.$api.add('weishengYuyue', {
									yonghuId: _this.user.id,
									weishengId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/weishengYuyue/list');
							}
						}
					});
				},
