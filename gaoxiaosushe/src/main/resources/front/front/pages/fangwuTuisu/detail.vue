	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('fangwuTuisu', this.fangwuTuisuId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fangwuTuisuPhoto ? this.detail.fangwuTuisuPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('fangwuTuisuId', _this.fangwuTuisuId);
                                // _this.$utils.jump('/pages/fangwuTuisuYuyue/add-or-update');

								await _this.$api.add('fangwuTuisuYuyue', {
									yonghuId: _this.user.id,
									fangwuTuisuId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/fangwuTuisuYuyue/list');
							}
						}
					});
				},
