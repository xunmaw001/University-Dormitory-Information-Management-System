	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.susheguanliyuanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('susheguanliyuan', this.susheguanliyuanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.susheguanliyuanPhoto ? this.detail.susheguanliyuanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('susheguanliyuanId', _this.susheguanliyuanId);
                                // _this.$utils.jump('/pages/susheguanliyuanYuyue/add-or-update');

								await _this.$api.add('susheguanliyuanYuyue', {
									yonghuId: _this.user.id,
									susheguanliyuanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/susheguanliyuanYuyue/list');
							}
						}
					});
				},
