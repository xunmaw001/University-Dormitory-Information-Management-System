	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.liuyanName" type="text" placeholder="留言标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('liuyan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('liuyan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='liuyanTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in liuyanTypesList" :key="index" :class="liuyanTypes===item.codeIndex?'tabActive':''"
                                  @tap="liuyanTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
            ...
            <view class="wrapper" style="padding: 10rpx 3rpx;margin-left: 25upx;margin-top: 10upx;">
                <view style="font-size: 30rpx;display: flex;justify-content: space-between;">
                 ...
                 <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 30%;margin-right: 5%;">留言类型</text>
                     <text style="width: 65%">
                         <text v-if="item.liuyanTypes != null">{{item.liuyanValue}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">留言内容</text>
						 <view style="width: 70%" v-html="item.liuyanContent"></view>
                 </view>
                <text style="float: right;padding: 10rpx 20rpx 20rpx;font-size: 20rpx;color:#8799A3;">留言时间:{{item.insertTime}}</text>
                 <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 35%;margin-right: 5%;">回复内容</text>
					 <view style="width: 60%" v-html="item.liuyanHuifuContent"></view>
                 </view>
                <text style="float: right;padding: 10rpx 20rpx 20rpx;font-size: 20rpx;color:#8799A3;">回复时间:{{item.updateTime}}</text>
            </view>
        </view>
	</view>
        <!--   报表    -->
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 留言类型 字段 字典表查询方法
            async liuyanTypesSelect(){
                let liuyanTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'liuyan_types',
                });
                liuyanTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.liuyanTypesList = liuyanTypes.data.list
            },
            //当前表的 留言类型字段类别搜索
			liuyanTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.liuyanTypes != 0) {
                    params.liuyanTypes = this.liuyanTypes
                }
                let _this = this
                let res = await this.$api.page(`liuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('liuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.liuyanShenheTime=shijian;
                            await _this.$api.update(`liuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.liuyanName) {
                    searchForm['liuyanName'] = this.searchForm.liuyanName
                }

                let res = await this.$api.page(`liuyan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.liuyanContent!=null){
                        item.liuyanContent = item.liuyanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
