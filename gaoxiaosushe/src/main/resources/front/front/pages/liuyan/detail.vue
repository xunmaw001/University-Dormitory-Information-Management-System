	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('liuyan', this.liuyanId);
                    this.detail = res.data
					this.detail.liuyanContent = this.detail.liuyanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					this.detail.liuyanHuifuContent = this.detail.liuyanHuifuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.liuyanPhoto ? this.detail.liuyanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('liuyanId', _this.liuyanId);
                                // _this.$utils.jump('/pages/liuyanYuyue/add-or-update');

								await _this.$api.add('liuyanYuyue', {
									yonghuId: _this.user.id,
									liuyanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/liuyanYuyue/list');
							}
						}
					});
				},
