	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('fangke', this.fangkeId);
                    this.detail = res.data
					this.detail.fangkeContent = this.detail.fangkeContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.fangkePhoto ? this.detail.fangkePhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('fangkeId', _this.fangkeId);
                                // _this.$utils.jump('/pages/fangkeYuyue/add-or-update');

								await _this.$api.add('fangkeYuyue', {
									yonghuId: _this.user.id,
									fangkeId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/fangkeYuyue/list');
							}
						}
					});
				},
