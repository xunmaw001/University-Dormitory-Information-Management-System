	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.jiaofeiName" type="text" placeholder="缴费标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('jiaofei','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
	...
	<view style="display: flex;flex-wrap: wrap">
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='jiaofeiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in jiaofeiTypesList" :key="index" :class="jiaofeiTypes===item.codeIndex?'tabActive':''"
                                  @tap="jiaofeiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                ...
                <!-- 当前表 -->
                 <view v-show="item.jiaofeiUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">缴费编号</text>
                     <text style="width: 70%">
                         <text v-if="item.jiaofeiUuidNumber != null">{{item.jiaofeiUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.jiaofeiMoney" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">缴费金额</text>
                     <text style="width: 70%">
                         <text v-if="item.jiaofeiMoney != null">{{item.jiaofeiMoney}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.jiaofeiContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">缴费缘由</text>
                 ...
                 <view v-show="item.jiaofeiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">缴费状态</text>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
				...
				<view v-if="item.jiaofeiTypes==1" @tap="jiaofei(item.id)" style="display: flex; width: 80%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 缴费状态字段类别搜索
			jiaofeiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.jiaofeiTypes != 0) {
                    params.jiaofeiTypes = this.jiaofeiTypes
                }
                let _this = this
                let res = await this.$api.list(`jiaofei`, params);
			...
			jiaofei(id) {
			    ...
			    uni.showModal({
					...
					await _this.$api.requestConditionDataGet('jiaofei','woyaojiaofei',null,data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
			        }
			...
			async search() {
                ...
                let res = await this.$api.list(`jiaofei`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.jiaofeiContent!=null){
                        item.jiaofeiContent = item.jiaofeiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
