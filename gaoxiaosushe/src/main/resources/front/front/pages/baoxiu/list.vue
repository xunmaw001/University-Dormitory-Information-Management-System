	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.baoxiuName" type="text" placeholder="报修标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('baoxiu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
	...
	<view style="display: flex;flex-wrap: wrap">
                    ...
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='baoxiuTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in baoxiuTypesList" :key="index" :class="baoxiuTypes===item.codeIndex?'tabActive':''"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                ...
                <!-- 当前表 -->
                 <view v-show="item.baoxiuUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报修编号</text>
                     <text style="width: 70%">
                         <text v-if="item.baoxiuUuidNumber != null">{{item.baoxiuUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.baoxiuFile" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">附件</text>
                     <view style="width: 70%" @tap="download(baseUrl+item.baoxiuFile)" class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>下载</view>
                 </view>
                ...
                <view v-show="item.baoxiuContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">报修内容</text>
                ...
                <view v-show="item.baoxiuHuifuContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">回复内容</text>
                    <view style="width: 70%" v-html="item.baoxiuHuifuContent"></view>
                </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`baoxiu`, params);
			...
			async search() {
                ...
                let res = await this.$api.page(`baoxiu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.baoxiuContent!=null){
                        item.baoxiuContent = item.baoxiuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
